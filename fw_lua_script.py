...
def decrypt(input_path: str, output_path: str) -> str:
    """
    Decompile Lua 5.1 bytecode to readable source using unluac.jar.
...
def gui_main():
    ...
    tk.Label(frm_tools, text='Tools:', bg='#12121c', fg=MUTE,
    ...
    def open_preview(source: str, filename: str):
        ...
        tk.Label(bar, text=f'{source.count(chr(10)) + 1} lines', bg='#313244', fg=MUTE,
                 font=('Segoe UI', 8)).pack(side='left', padx=12)
        tk.Button(bar, text='Close', bg=SUB, fg=FG, relief='flat',
                  font=('Segoe UI', 8), cursor='hand2',
    ...
    def do_preview():
        ...
        if not inp:
            messagebox.showwarning('Warning', 'Please select an input file.', parent=root)
            return
        try:
            ...
            messagebox.showerror('Preview Error', str(e), parent=root)
    ...
    def do_decrypt():
        ...
        if not inp:
            messagebox.showwarning('Warning', 'Please select an input file.', parent=root); return
        if not out:
            out = auto_decrypt_output(inp); var_dec_out.set(out)
        try:
            decrypt(inp, out)
            log_write(f'\u2713  Decrypted \u2192 {Path(out).name}', 'ok')
        except Exception as e:
            log_write(f'\u2717  {e}', 'err')
    ...
    def do_encrypt():
        ...
        try:
            ...
            log_write(f'\u2713  Encrypted \u2192 {Path(out).name}  ({size} bytes)', 'ok')
        except Exception as e:
            log_write(f'\u2717  {e}', 'err')
    ...
    def startup_check():
        ...
        if missing:
            log_write("\u26a0  Missing tools \u2014 place them in the same folder as the program:", 'warn')
            for line in missing:
                log_write(line, 'warn')
        else:
            log_write("\u2713  All tools found. Ready to use.", 'ok')
